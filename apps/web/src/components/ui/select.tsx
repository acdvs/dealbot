          ...
          <Select.Popup className="scrollbar max-h-75 w-[calc(100vw-var(--edge-gap)*2)] xs:w-72 group border border-foreground-accent origin-top overflow-y-auto rounded-md bg-background-accent py-1 text-forground transition-[transform,scale,opacity] data-ending-style:scale-90 data-ending-style:opacity-0 data-ending-style:transition-none data-starting-style:scale-90 data-starting-style:opacity-0 data-[side=none]:data-starting-style:scale-100 data-[side=none]:data-starting-style:opacity-100 data-[side=none]:data-starting-style:transition-none">
            {items.map(({ label, value }) => (
              <Select.Item
                ...
                disabled={!value}
                className={cx(
                  'flex justify-between items-center gap-3 px-3 py-1 cursor-default outline-none select-none group-data-[side=none]:max-w-72 group-data-[side=none]:pr-12 group-data-[side=none]:text-base data-highlighted:relative data-highlighted:z-0 data-highlighted:before:absolute data-highlighted:before:inset-x-1 data-highlighted:before:inset-y-0 data-highlighted:before:z-[-1] data-highlighted:before:rounded-sm',
                  value
