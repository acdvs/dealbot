  ...
  if (option === 'waitlisted' || option === 'collected') {
    ...
    embed = new Discord.MessageEmbed({
      ...
      fields: [
        {
          name: 'Game',
          value: list.map(x => `${x.position}. ${trimTitle(x.title)}`).join('\n'),
          inline: true
        },
        {
          name: 'Count',
    ...
    embed = new Discord.MessageEmbed({
      ...
      description: [
        ...
        popularities.map(x => `${x.position}. ${trimTitle(x.title)}`).join('\n')
      ].join('\n')
    });
  }
...
/**
 * Trim titles to a certain length
 * @param {string} title
 * @returns {string}
 */
function trimTitle(title) {
  return title.length > TITLE_CUTOFF ? `${title.substring(0, TITLE_CUTOFF)}...` : title;
}
