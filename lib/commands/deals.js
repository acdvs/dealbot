...
module.exports = {
  ...
  run: (msg, game) => getDeals(msg, game)
};
 ...
 */
async function getDeals(msg, game) {
  if (!game) return;
...
async function generateEmbed(msg, game, gameId) {
  try {
    ...
    const gameDeals = await apiUtil.getGameDeals(gameId, ignoredSellers);
    const gameInfo = await apiUtil.getGameInfo(gameId);

    const list = gameDeals?.list.filter(x => x.price_new < x.price_old);

    if (!list || list.length === 0) {
      msg.channel.send(`There are currently no deals on ${gameInfo.title || game}.`);
      return;
    }
    ...
    if (sellers.join('\n').length > CHAR_LIMIT) {
      ...
      const getOverflowText = currRowCount => `[...and ${sellers.length - currRowCount} more deals](${gameDeals.urls.game})`;
