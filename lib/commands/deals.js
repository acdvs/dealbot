...
async function generateEmbed(msg, game, gameId) {
  try {
    ...
    if (!gameDeals || !gameInfo) {
      msg.channel.send(createBasicEmbed(`Unable to get info from IsThereAnyDeal for ${gameTitle}. Try again later.`));
      return;
    }
    ...
    if (!list || list.length === 0) {
      msg.channel.send(createBasicEmbed(`There are currently no deals on ${gameTitle}.`));
      return;
    }
