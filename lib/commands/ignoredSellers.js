  ...
  try {
    if (!options) {
      ...
      if (ignoredSellers?.length > 0) {
        ...
        msg.channel.send(createBasicEmbed('No ignored sellers have been added.'));
      }
    ...
    if (seller) {
      ...
      if (operation === 'add') {
        if (!hasIgnoredSeller) {
          ...
          msg.channel.send(createBasicEmbed('That seller is already ignored.'));
        }
      } else if (operation === 'remove') {
        if (hasIgnoredSeller) {
          changed = await db.removeIgnoredSeller(msg.guild.id, seller);
        } else {
          msg.channel.send(createBasicEmbed('That seller is not ignored.'));
        }
      }
