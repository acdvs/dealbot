...
bot.on('message', (msg) => {
  ...
  if (timestamps.has(msg.guild.id)) {
    ...
    if (now < expirationTime) {
      ...
      msg.channel.send(`The \`${PREFIX}${command.name}\` command can be used again in ${timeLeft.toFixed(1)} second${s}.`);
  ...
  try {
    ...
    msg.channel.send('Unable to process that command. Try again in several seconds.');
  }
});
